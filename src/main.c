...
static void draw_fallback(vita2d_texture *t,Rect p,float cam,int left){
 ...
 float h=112.0f,sc=h/th,w=tw*sc,x=p.x-cam+(p.w-w)*0.5f,y=p.y+p.h-h+4.0f;
 if(left)vita2d_draw_texture_scale(t,x+w,y,-sc,sc);else vita2d_draw_texture_scale(t,x,y,sc,sc);
}

static void draw_atlas_frame(vita2d_texture *atlas,vita2d_texture *fallback,Rect p,float cam,int left,int col,int row){
 if(!atlas){ draw_fallback(fallback,p,cam,left); return; }
 float aw=(float)vita2d_texture_get_width(atlas),ah=(float)vita2d_texture_get_height(atlas);
 float fw=aw/3.0f,fh=ah/3.0f;
 float out_h=118.0f,sc=out_h/fh,out_w=fw*sc;
 float x=p.x-cam+(p.w-out_w)*0.5f,y=p.y+p.h-out_h+7.0f;
 float sx=fw*(float)col,sy=fh*(float)row;
 if(left) vita2d_draw_texture_part_scale(atlas,x+out_w,y,sx,sy,fw,fh,-sc,sc);
 else vita2d_draw_texture_part_scale(atlas,x,y,sx,sy,fw,fh,sc,sc);
}
...
int main(void){
 ...
 Rect goal={3480,350,56,120};int gold=0,dead=0,won=0,intro=1,running=1,left=0,land_timer=0;float cam=0;unsigned int tick=0,old=0;SceCtrlData pad;memset(&pad,0,sizeof(pad));
 while(running){tick++;sceCtrlPeekBufferPositive(0,&pad,1);unsigned int pressed=pad.buttons&~old;if(pressed&SCE_CTRL_START)running=0;
  if(intro){if(pressed&SCE_CTRL_CROSS)intro=0;}
   ...
   float oy=player.y;int was_ground=ground;vy+=GRAVITY;if(vy>18)vy=18;player.y+=vy;ground=0;
   ...
   if(!was_ground&&ground)land_timer=8;else if(land_timer>0)land_timer--;
  ...
  if(intro){vita2d_draw_rectangle(100,80,760,360,RGBA8(0,0,0,190));vita2d_pgf_draw_text(font,250,145,RGBA8(255,255,255,255),1.7f,"PLATFORMER TEST");vita2d_pgf_draw_text(font,220,220,RGBA8(235,235,235,255),1.0f,"LEFT STICK: MOVE");vita2d_pgf_draw_text(font,220,265,RGBA8(235,235,235,255),1.0f,"X: JUMP / STOMP ENEMIES");vita2d_pgf_draw_text(font,345,390,RGBA8(200,215,235,255),1.0f,"PRESS X TO START");}
   ...
   int col=0,row=0;if(!ground){row=2;col=vy<0?0:1;}else if(land_timer>0){row=2;col=2;}else if(fabsf(vx)>.3f){row=1;col=(tick/6)%3;}else{row=0;col=(tick/18)%3;}draw_atlas_frame(atlas,fallback,player,cam,left,col,row);
